...
TowerBuilder.prototype.buildTowerFromPrefab = function(prefab) {
    ...
    famobi.log("Added " + this.entity.totalItems + ' bodies, ' + this.entity.requiredItems + ' required, tower height ', this.towerHeight);
...
BallController.prototype.verifyBallColor = function() {
   ...
   if(availableColors.indexOf(this.preparedBall.materialIndex) === -1) {
       famobi.log("ball color replaced");
...
PanoramicCamera.attributes.add('distanceMax', {type: 'number', default: 0, title: 'Distance Max', description: 'Setting this at 0 will give an infinite distance limit'});
PanoramicCamera.attributes.add('distanceMin', {type: 'number', default: 0, title: 'Distance Min'});
PanoramicCamera.attributes.add('pitchAngleMax', {type: 'number', default: 90, title: 'Pitch Angle Max (degrees)'});
PanoramicCamera.attributes.add('pitchAngleMin', {type: 'number', default: -90, title: 'Pitch Angle Min (degrees)'});
...
// Moves the camera to look at an entity and all its children so they are all in the view
PanoramicCamera.prototype.focus = function (focusEntity) {
    // Calculate an bounding box that encompasses all the models to frame in the camera view
    this._buildAabb(focusEntity, 0);
...
PanoramicCamera.distanceBetween = new pc.Vec3();

// Set the camera position to a world position and look at a world position
// Useful if you have multiple viewing angles to swap between in a scene
PanoramicCamera.prototype.resetAndLookAtPoint = function (resetPoint, lookAtPoint) {
    this.pivotPoint.copy(lookAtPoint);
    this.entity.setPosition(resetPoint);

    this.entity.lookAt(lookAtPoint);

    var distance = PanoramicCamera.distanceBetween;
    distance.sub2(lookAtPoint, resetPoint);
    this.distance = distance.length();
...
// Set camera position to a world position and look at an entity in the scene
// Useful if you have multiple models to swap between in a scene
PanoramicCamera.prototype.resetAndLookAtEntity = function (resetPoint, entity) {
    this._buildAabb(entity, 0);
    this.resetAndLookAtPoint(resetPoint, this._modelsAabb.center);
};
...
// Set the camera at a specific, yaw, pitch and distance without inertia (instant cut)
PanoramicCamera.prototype.reset = function (yaw, pitch, distance) {
    this.pitch = pitch;
    this.yaw = yaw;
    this.distance = distance;
...
PanoramicCamera.prototype.update = function(dt) {
    // Add inertia, if any
    var t = this.inertiaFactor === 0 ? 1 : Math.min(dt / this.inertiaFactor, 1);
    this._distance = pc.math.lerp(this._distance, this._targetDistance, t);
    this._yaw = pc.math.lerp(this._yaw, this._targetYaw, t);
...
PanoramicCamera.prototype._buildAabb = function (entity, modelsAdded) {
    var i = 0;
...
PanoramicCamera.prototype._calcYaw = function (quat) {
    var transformedForward = new pc.Vec3();
    quat.transformVector(pc.Vec3.FORWARD, transformedForward);
...
PanoramicCamera.prototype._clampDistance = function (distance) {
    if (this.distanceMax > 0) {
        return pc.math.clamp(distance, this.distanceMin, this.distanceMax);
    } else {
        return Math.max(distance, this.distanceMin);
...
PanoramicCamera.prototype._clampPitchAngle = function (pitch) {
    // Negative due as the pitch is inversed since the camera is orbiting the entity
    return pc.math.clamp(pitch, -this.pitchAngleMax, -this.pitchAngleMin);
};


PanoramicCamera.quatWithoutYaw = new pc.Quat();
PanoramicCamera.yawOffset = new pc.Quat();

PanoramicCamera.prototype._calcPitch = function(quat, yaw) {
    var quatWithoutYaw = PanoramicCamera.quatWithoutYaw;
    var yawOffset = PanoramicCamera.yawOffset;
...
MousePanoramicInput.prototype.initialize = function() {
    this.panoramicCamera = this.entity.script.panoramicCamera;
        
    if (this.panoramicCamera) {
        var self = this;
...
MousePanoramicInput.fromWorldPoint = new pc.Vec3();
MousePanoramicInput.toWorldPoint = new pc.Vec3();
MousePanoramicInput.worldDiff = new pc.Vec3();


MousePanoramicInput.prototype.pan = function(screenPoint) {
    var fromWorldPoint = MousePanoramicInput.fromWorldPoint;
    var toWorldPoint = MousePanoramicInput.toWorldPoint;
    var worldDiff = MousePanoramicInput.worldDiff;
    
    // For panning to work at any zoom level, we use screen point to world projection
    // to work out how far we need to pan the pivotEntity in world space 
    var camera = this.entity.camera;
    var distance = this.panoramicCamera.distance;
    ...
    this.panoramicCamera.pivotPoint.add(worldDiff);    
};


MousePanoramicInput.prototype.onMouseDown = function (event) {
    switch (event.button) {
        case pc.MOUSEBUTTON_LEFT: {
            this.lookButtonDown = true;
        } break;
...
MousePanoramicInput.prototype.onMouseUp = function (event) {
    switch (event.button) {
        case pc.MOUSEBUTTON_LEFT: {
            this.lookButtonDown = false;
        } break;
...
MousePanoramicInput.prototype.onMouseMove = function (event) {    
    var mouse = pc.app.mouse;
    if (this.lookButtonDown) {
        this.panoramicCamera.pitch -= event.dy * this.orbitSensitivity;
        this.panoramicCamera.yaw -= event.dx * this.orbitSensitivity;
...
MousePanoramicInput.prototype.onMouseWheel = function (event) {
    this.panoramicCamera.distance -= event.wheel * this.distanceSensitivity * (this.panoramicCamera.distance * 0.1);
    event.event.preventDefault();
};


MousePanoramicInput.prototype.onMouseOut = function (event) {
    this.lookButtonDown = false;
    this.panButtonDown = false;
};
...
ScaleManager.prototype.measurePerformance = function() {
    ...
    if(ScaleManager.savedQuality === undefined) {
        famobi.log('Measuring performance...');
    ...
    if(this.customMeasurementStarted) {
        ...
        if(this.currentMeasurementSession.frames >= this.framesPerIteration) {
            ...
            if(this.customMeasurementIteration >= this.qualityPresets.length * this.numIterations) {
                ...
                famobi.log("Performance measurements finished: ", this.customMeasurementResults);
                ...
                famobi.log('Quality set to ' + ['Low', 'Medium', 'High'][ScaleManager.qualityIndex]);
            }
        }
    }
};
...
SoundController.prototype.playSound = function(key, debounceDelay) {
    if(debounceDelay) {
         ...
         var lastTimestamp = this.app.root.findByName("SoundStorage").sound.slot(key).lastTimeStamp;
         if(lastTimestamp && currentTimestamp - lastTimestamp < debounceDelay) {
             return;
         }
         this.app.root.findByName("SoundStorage").sound.slot(key).lastTimeStamp = currentTimestamp;
    }
    this.app.root.findByName("SoundStorage").sound.play(key);
};

SoundController.prototype.stopSound = function(key) {
    this.app.root.findByName("SoundStorage").sound.stop(key);
};

SoundController.prototype.muteSound = function(key) {
    this.app.root.findByName("SoundStorage").sound.slot(key).volume = 0;
};

SoundController.prototype.unmuteSound = function(key, volume) {
    this.app.root.findByName("SoundStorage").sound.slot(key).volume = volume;
};
...
SettingsPanel.prototype.initialize = function() {
    ...
    Utils.assignAction(this.entity.buttonQualityLow, this.rescalePressed, this);
    Utils.assignAction(this.entity.buttonQualityMedium, this.rescalePressed, this);
    Utils.assignAction(this.entity.buttonQualityHigh, this.rescalePressed, this);
...
SettingsPanel.prototype.update = function(dt) {
    ...
    this.entity.panel.enabled = this.entity.buttonSettings.enabled = !(autoQuality || externalMute);
};
...
BallsCounter.prototype.update = function(dt) {   
     ...
     if(preparedBall && !preparedBall.script.ball.isColorBall && !preparedBall.script.ball.isCannonBall && this.lastBallsAmount > 0) {
         ...
         this.entity.setLocalPosition((this.screenPos.x * ScaleManager.qualityFactor) / scale, (device.height - (this.screenPos.y * ScaleManager.qualityFactor)) / scale, 0);    
     } else {
         this.ballsCounterText.enabled = false;         
